...
@include('frontend.includes.breadcrumbs',['headtitle'=>'Our Offerings','title'=>'Our Offerings'])

<!-- Service Box -->
@if(count($data['all_offerings'])>0)
<section class="service_box">
    <div class="container">
        <div class="min_service_box_area">
            <div class="row justify-content-center">
                @foreach($data['all_offerings'] as $offerings)
                <div class="col-xl-4 service-div mb-5">
                <a href="{{route('frontend.service_detail',$offerings->slug)}}">
                    <div class="single_service_box wow fadeInLeft" data-wow-delay="0.3s">
                        <img src="{{asset('images/offerings/'.$offerings->logo)}}" style="min-width:100%;" alt="">
                        <h4>{{$offerings->title}}</h4>
                        <p>{{$offerings->slogan}}</p>
                        <a href="{{route('frontend.service_detail',$offerings->slug)}}" class="text_btn">Learn more <i
